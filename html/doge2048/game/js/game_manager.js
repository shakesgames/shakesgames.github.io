...
GameManager.prototype.restart = function () {
  ...
  // Dispatch game restart event
  var gameRestartEvent = new CustomEvent('gameRestart', {
    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && next.value === tile.value && !next.mergedFrom) {
          ...
          // Dispatch high score tile events for ad integration
          if (merged.value >= 512) {
            var highScoreTileEvent = new CustomEvent('highScoreTile', {
              detail: {
                tileValue: merged.value,
                currentScore: self.score,
              },
            })
            document.dispatchEvent(highScoreTileEvent)
          }
        } else {
